...
FUNCTION_BLOCK MC_003_GearInPos (*commands a gear ratio between the position of the slave and master axis*)
	...
	VAR
		...
		C_Error : BOOL; (*internal variable*)
		C_ErrorID : UINT; (*internal variable*)
		Reserve1 : INT; (*internal variable*)
		CC_ErrorID : UINT; (*internal variable*)
		CC_Error : BOOL; (*internal variable*)
		C_Velocity : REAL; (*internal variable*)
		MaPosition : DINT; (*internal variable*)
...
FUNCTION_BLOCK MC_070BR_CheckAutCompensation (*checks the compensation data of a cam profile automat*)
	VAR_INPUT
		Master : UDINT; (*axis reference of the master axis*)
		Slave : UDINT; (*axis reference of the slave axis*)
		Execute : BOOL; (*check the compensation data at rising edge*)
...
FUNCTION_BLOCK MC_071BR_TorqueControl (*starts a torque movement with limited speed*)
	VAR_INPUT
		Axis : UDINT;
		Enable : BOOL;
		InitData : BOOL;
...
FUNCTION_BLOCK MC_072BR_AxisErrorCollector (*collects information if a FB or axis error occured and if an axis is in state Errorstop*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Enable : BOOL; (*function block is activated*)
	END_VAR
...
FUNCTION_BLOCK MC_073BR_ReadAxisError (*reads and acknowledges an axis error record and reads the error text*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Enable : BOOL; (*read error record/text*)
		Acknowledge : BOOL; (*acknowledge current axis error*)
...
FUNCTION_BLOCK MC_074BR_DigitalCamSwitch (*allows a digital output (Track) to be set whenever the axis is inside a position window*)
	VAR_INPUT
		Enable : BOOL; (*when set the FB parameters are downloaded*)
		Switches : MC_MASTER_SWITCHES_REF; (*reference to the switching actions*)
		OutputOptions : MC_OUTPUT_OPTIONS_REF; (*reference to the output options*)
...
FUNCTION_BLOCK MC_075BR_GetParIDInfo (*determines data type and data length of a ParID*)
	VAR_INPUT
		Execute : BOOL; (*determine ParID information at rising edge*)
		ParID : UINT; (*ParID of which the information should be determined*)
	END_VAR
...
FUNCTION_BLOCK MC_076BR_SetHardwareInputs (*forces the hardware inputs*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Enable : BOOL; (*set the status of the hardware inputs as long as the input is enabled*)
		HomeSwitch : BOOL; (*if TRUE the reference hardware input is activated*)
...
FUNCTION_BLOCK MC_077BR_BrakeControl (*executes several commands for holding brake*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Execute : BOOL; (*execute brake command at rising edge*)
		Command : USINT; (*brake command*)
...
FUNCTION_BLOCK MC_078BR_InitSendParID (*initializes sending of a ParID on a drive*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Execute : BOOL; (*initialize sending of ParID at the rising edge*)
		ParID : UINT; (*ParID that should be sent by the drive*)
...
FUNCTION_BLOCK MC_079BR_InitReceiveParID (*initializes receiving of a ParID on a drive*)
	VAR_INPUT
		Master : UDINT; (*axis reference*)
		Slave : UDINT; (*axis reference*)
		Execute : BOOL; (*initialize receiving of ParID at the rising edge*)
...
FUNCTION_BLOCK MC_080BR_CalcSectionsFromCam
	VAR_INPUT
		Execute : BOOL; (*calculation starts at rising edge*)
		DataObjectName : STRING[12]; (*name of the cam table*)
		DataAddress : UDINT; (*address of the variable for the sections*)
...
FUNCTION_BLOCK MC_081BR_InitReceiveNetworkData (*initializes receiving of 4-Byte data from a network station*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Execute : BOOL; (*initialize receiving of data at rising edge*)
		NodeNumber : USINT; (*node number of sending network station*)
		BitOffset : UINT; (*bit offset of the data inside the telegram*)
		DataType : UINT; (*data type of the received data*)
		InterpolationMode : USINT; (*interpolation mode of value on receiving drive*)
...
FUNCTION_BLOCK MC_082BR_SaveCamProfileObj (*saves the data of a cam profile into a data module*)
	VAR_INPUT
		Execute : BOOL; (*save the cam profile at rising edge*)
		DataObjectName : STRING[12]; (*name for the data module in which the cam profile is saved*)
		DataAddress : UDINT; (*address of the cam profile data*)
		DataObjectVersion : UINT; (*version for the data module (is displayed in AutomationStudio)*)
	END_VAR
	VAR_OUTPUT
		Done : BOOL; (*data module was saved*)
		Busy : BOOL; (*function block is not finished*)
...
FUNCTION_BLOCK MC_083BR_JogVelocity (*enables execution of jog movements*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Enable : BOOL; (*enables execution of jog movements*)
		Velocity : REAL; (*maximum velocity*)
...
FUNCTION_BLOCK MC_084BR_JogLimitPosition (*enables and executes jog movements between two limit positions*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Enable : BOOL; (*enables execution of jog movements*)
		Velocity : REAL; (*maximum velocity*)
...
FUNCTION_BLOCK MC_085BR_JogTargetPosition (*enables and executes jog movements with a defined target position*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Enable : BOOL; (*enables executeion of jog movements*)
		Velocity : REAL; (*maximum velocity*)
...
FUNCTION_BLOCK MC_086BR_GetParIDTransferInfo (*determines ParID transfer information of an axis*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Execute : BOOL; (*determine ParID transfer information at rising edge*)
	END_VAR
...
FUNCTION_BLOCK MC_087BR_GearIn
	VAR_INPUT
		Master : UDINT; (*axis reference of the master axis *)
		Slave : UDINT; (*axis reference of the slave axis *)
		Enable : BOOL; (*the gear ratio is active as long as this input is set*)
...
FUNCTION_BLOCK MC_088BR_OffsetVelocity (*adds a velocity offset to the velocity of the slave axis*)
	VAR_INPUT
		Slave : UDINT; (*axis reference of the slave axis*)
		Enable : BOOL; (*enable the function block*)
		CyclicVelocity : REAL; (*additive velocity*)
...
FUNCTION_BLOCK MC_089BR_OffsetZone (*adds an offset to the position of the slave axis*)
	VAR_INPUT
		Slave : UDINT; (*axis reference of the slave axis*)
		Enable : BOOL; (*enable the function block*)
		InitData : BOOL; (*initialize the input data*)
...
FUNCTION_BLOCK MC_090BR_SetupSynchronMotor (*starts the motorsetup for a synchron motor and saves the parameters*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Execute : BOOL; (*starts the commanded action at rising edge*)
		Command : UINT; (*choose the command for the setup*)
...
FUNCTION_BLOCK MC_091BR_CheckEndlessPosition (*checks the endless position data for restoring the axis position*)
    VAR_INPUT
        Axis : UDINT; (*axis reference*)
        Execute : BOOL; (*check the endless position data at rising edge*)
        DataAddress: UDINT; (*data address of momory storage for endless position*)
...
FUNCTION_BLOCK MC_092BR_LimitLoad (*limits the motor torque to set values*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Enable : BOOL; (*when set limitation is activated*)
		InitData : BOOL; (*initialize the input data*)
...
FUNCTION_BLOCK MC_093BR_LimitLoadCam (*limits the motor torque to value from cam profile*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Enable : BOOL; (*when set limitation is activated*)
		InitData : BOOL; (*initialize the input data*)
...
FUNCTION_BLOCK MC_095BR_ConfigPowerStageCheck (*configurates the power stage checks of the ACOPOS*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Execute : BOOL; (*transfer configuration at rising edge *)
		Configuration : MC_POWER_STAGE_CHECK_REF; (*configuration data*)
...
FUNCTION_BLOCK MC_094BR_ResetAutPar (*initializes the general parameters of the cam automat*)
	VAR_INPUT
		Axis : UDINT; (*axis reference*)
		Execute : BOOL; (*set parameter to default values at rising edge*)
		GlobalParams : USINT; (*selection of the global parameter*)
